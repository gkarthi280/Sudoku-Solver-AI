...
class BTSolver:
        ...
        while len(assignedVars) != 0:
            ...
            for neighbor in self.network.getNeighborsOfVariable(av):
                if neighbor.isChangeable and not neighbor.isAssigned() and neighbor.getDomain().contains(av.getAssignment()):
                    ...
                    if neighbor.domain.size() == 1:
                        neighbor.assignValue(neighbor.domain.values[0])
                        modified[neighbor] = neighbor.domain.values[0]
                        assignedVars.append(neighbor)
                        
        # Check 2
        numVals = self.gameboard.p * self.gameboard.q
        for c in self.network.constraints:
            counter = [0] * numVals
            for var in c.vars:
            ...
            for i in range(len(counter)):
                ...
                if counter[i] == 1:
                    for variable in c.vars:
                        if variable.isChangeable and not variable.isAssigned() and variable.getDomain().contains(i+1):
                            self.trail.push(variable)
                            variable.assignValue(i+1)
                            modified[variable] = i+1
    ...
    def getMRV ( self ):
        ...
        for c in self.network.constraints:
            for v in c.vars:
                if not v.isAssigned():
                    if (val == None or v.getDomain().size() < val):
                        val = v.getDomain().size()
                        var = v
        return var
    ...
    def getValuesLCVOrder ( self, v ):
        ...
        def LCV(value):
            affects = 0
            for neighbor in self.network.getNeighborsOfVariable(v):
                if (not neighbor.isAssigned() and value in neighbor.getValues()):
                    affects += 1
            return affects
                        
        return sorted(v.getValues(), key = lambda x : LCV(x))
